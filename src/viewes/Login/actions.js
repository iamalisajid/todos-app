...
export const loginUser = (user) => (dispatch) => {
  ...
  apiCaller(API_ROUTES.LOGIN, REQUEST_TYPE.GET, null, user).then(
    (response) =>
      response.data.length ? dispatch(loginSuccess(response.data)) : dispatch(loginfailed('Invalid Username Password'))
  );
};
